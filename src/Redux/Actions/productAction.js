...
export const AddProductsAction = (formData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post("http://localhost:8000/api/product/add", formData);
...
export const getAllProductsAction =
  ...
  async (dispatch) => {
    try {
      ...
      let UrlLink;
      if (keyword != "") {
        UrlLink = `http://localhost:8000/api/product/getAllProducts?keyword=${keyword}`;
      } else if (filterCategory && price) {
        const gte = price.split("-")[0];
        const lte = price.split("-")[1];
        UrlLink = `http://localhost:8000/api/product/getAllProducts?category=${filterCategory}&gte=${gte}&lte=${lte}`;
      } else {
        UrlLink = "http://localhost:8000/api/product/getAllProducts";
      }
...
export const deleteProductAction = (productId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`http://localhost:8000/api/product/delete/${productId}`);
