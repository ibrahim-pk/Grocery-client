...
export const getUsersOrdersAction = (userId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:8000/api/user/my/all/orders/${id}`);
...
export const getUsersOrderDetailsAction = (orderId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:8000/api/user/my/order/${orderId}`);
...
export const getAllOrdersAdminAction = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("http://localhost:8000/api/user/admin/orders");
...
export const updateOrdersAdminAction =
  (orderId, oStatus) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.put(`http://localhost:8000/api/user/update/order/${orderId}`, {
        oStatus,
      });
