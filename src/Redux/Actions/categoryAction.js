...
export const addCategoryAction = (categoryData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post("http://localhost:8000/api/category/add", categoryData);
...
export const getAllCategoryAction = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("http://localhost:8000/api/category/get");
...
export const deleteCategoryAction = (categoryId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`http://localhost:8000/api/category/delete/${categoryId}`);
